...
export default function Customers() {
  ...
  // Windowed LTV + new customer counts -- driven by cacWindow selector
  const windowStats        = subs.data?.windowedStats?.[cacWindow] || {};
  const windowNewCustomers = windowStats.newCustomers || 0;
  const windowAvgLtv       = windowStats.avgLtv || 0;
  const ltvDollars         = windowAvgLtv / 100;

  // CAC: QB marketing spend for selected window ÷ new customers in same window
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Acquisition window selector -- affects LTV, CAC, and LTV:CAC */}
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted">Acquisition window -- affects LTV, CAC, and LTV:CAC</p>
        <div className="flex items-center gap-1 bg-bg rounded-lg p-1 border border-border">
          {WINDOW_OPTIONS.map(({ value, label }) => (
            <button
              key={value}
              onClick={() => setCacWindow(value)}
              className={`px-3 py-1 rounded text-xs font-medium transition-colors ${
                cacWindow === value ? 'bg-purple text-white' : 'text-muted hover:text-white'
              }`}
            >
              {label}
            </button>
      ...
      <div className="grid grid-cols-5 gap-4">
        ...
        <StatCard
          ...
          value={ltvDollars > 0 ? fmtMrr(ltvDollars) : '--'}
          sub={`Customers acquired in last ${WINDOW_LABELS[cacWindow]}`}
          accent="white"
          tooltip={ltvDollars > 0
            ? `The average total revenue generated by customers who signed up in the last ${WINDOW_LABELS[cacWindow]}.\nTotal paid invoices from ${windowNewCustomers} customers ÷ ${windowNewCustomers} customers`
            : `No customers acquired in the last ${WINDOW_LABELS[cacWindow]}`}
          loading={subs.loading}
        />
        <StatCard
          label="Customer Acquisition Cost"
          value={qbConfigured && cac > 0 ? fmtMrr(cac) : '--'}
          sub={`${windowNewCustomers} new customers (last ${WINDOW_LABELS[cacWindow]})`}
          accent="white"
          tooltip={qbConfigured
            ? marketingAccounts.length > 0
              ? `How much it costs on average to acquire one new paying customer, based on the last ${WINDOW_LABELS[cacWindow]} of marketing spend.\n${fmtMrr(spend)} marketing spend ÷ ${windowNewCustomers} new customers\nAccounts: ${marketingAccounts.join(', ')}`
              : `How much it costs on average to acquire one new paying customer.\nNo marketing/advertising QB accounts found in the last ${WINDOW_LABELS[cacWindow]}`
