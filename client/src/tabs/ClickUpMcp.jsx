...
export default function ClickUpMcp() {
  ...
  async function loadSessions() {
    try {
      const res  = await fetch('/api/sessions/clickup', { credentials: 'include' });
  ...
  async function newSession() {
    ...
    try {
      const res = await fetch('/api/sessions/clickup', {
  ...
  async function selectSession(id) {
    ...
    setMessages([{ role: 'assistant', content: GREETING }]);
    try {
      const res  = await fetch(`/api/sessions/clickup/${id}/messages`, { credentials: 'include' });
      const data = await res.json();
  ...
  async function renameSession(id, name) {
    try {
      const res     = await fetch(`/api/sessions/clickup/${id}`, {
  ...
  async function deleteSession(id) {
    try {
      await fetch(`/api/sessions/clickup/${id}`, { method: 'DELETE', credentials: 'include' });
  ...
  async function send(text) {
    ...
    if (!sessionId) {
      ...
      try {
        const sr = await fetch('/api/sessions/clickup', {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
