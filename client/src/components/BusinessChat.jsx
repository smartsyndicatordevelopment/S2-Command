...
function renderContent(text) {
  ...
  return parts.map((part, i) => {
    if (part.type === 'code') {
      return (
        <pre key={i} className="my-2 p-3 rounded text-xs overflow-x-auto" style={{ backgroundColor: 'rgba(0,0,0,0.3)', color: 'var(--c-text-primary)' }}>
          {part.content}
        </pre>
      );
    }
    ...
    <div className="rounded-lg border overflow-hidden flex" style={{ borderColor: 'var(--c-border)', backgroundColor: 'var(--c-card)', height: '480px', ...style }}>
      ...
      <div className="flex flex-col border-r" style={{ width: '220px', borderColor: 'var(--c-border)', flexShrink: 0 }}>
        ...
        <div className="flex-1 overflow-y-auto">
          {chats.length === 0 ? (
            <p className="text-xs p-3" style={{ color: 'var(--c-muted)' }}>No saved chats yet.</p>
          ) : (
            chats.map(chat => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs mt-0.5" style={{ color: 'var(--c-dim)' }}>{relTime(chat.createdAt)}</p>
                  </div>
                  <button
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {messages.length === 0 && !sending && (
            <div className="flex flex-col items-center justify-center h-full gap-3" style={{ color: 'var(--c-dim)' }}>
              <Bot size={28} />
          ...
          {sending && (
            <div className="flex gap-3">
              <div className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center" style={{ backgroundColor: 'var(--c-avatar-bg)' }}>
                <Bot size={11} style={{ color: 'var(--c-dim)' }} />
              </div>
              <div className="px-3 py-2 rounded-lg text-xs" style={{ backgroundColor: 'var(--c-msg-ai-bg)', border: '0.5px solid var(--c-border)' }}>
                <Loader size={12} className="animate-spin" style={{ color: 'var(--c-muted)' }} />
              </div>
            </div>
          )}
